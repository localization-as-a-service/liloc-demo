...
from utils.interfaces import GlobalLiDARInterface, GlobalRegistrationInterface
...
def update_visualizer(global_lidar, grv, vis, current_t):
    target = global_lidar.get_global_pcd(current_t)

    if grv.found_correct_global:
        vis.set_global_pcd(target)
        vis.draw_trajectory(grv.get_global_t())


def update_visualizer_local(grv, vis, source, transformation_matrix):
...
def main():
    ...
    glb_lidar = GlobalLiDARInterface("http://localhost:5000")
    glb_reg_server = GlobalRegistrationInterface("http://localhost:5000")
    glb_reg_vfc = GlobalRegistrationVerification()

    if not glb_lidar.start():
    ...
    try:
        ...
        while True:
            ...
            glb_reg_vfc.update_local(current_t, device_local_t[-1])

            if glb_reg_vfc.found_correct_global:
                Thread(target=update_visualizer_local, args=(glb_reg_vfc, vis, source, copy.deepcopy(glb_reg_vfc.global_t[-1]))).start()
            ...
            if current_t - global_t > 1200:
                ...
                if not glb_reg_vfc.found_correct_global:
                    Thread(target=global_registration, args=(glb_lidar, glb_reg_server, glb_reg_vfc, current_t, source)).start()
                else:
                    Thread(target=update_visualizer, args=(glb_lidar, glb_reg_vfc, vis, current_t)).start()

                global_t = current_t
            
            if current_t - start_t > 60000:
