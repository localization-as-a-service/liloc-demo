...
def receive_data(socket, flags=0):
    md = socket.recv_json(flags=flags)
    return md['timestamp'], np.array(md['source']), np.array(md['target'])
...
if __name__ == '__main__':
    ...
    while True:
        try:
            timestamp, source, target = receive_data(s)
            print("Received array: ", timestamp, source.shape, target.shape)
        except KeyboardInterrupt:
            break
    print("Server Done")
    s.close()